...
Networks.livenet = Networks.add(require('coininfo')('DNGR').toBitcore());
...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for dongrid is necessary for this upgrade with the "reindex=1" dongri.conf option. \n' +
                 'There are changes necessary in both dongri.conf and bitcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/junkwind/bitcore-node/blob/dongrid/docs/upgrade.md \n');
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/dongrid')
            }
          }
        }
      };
